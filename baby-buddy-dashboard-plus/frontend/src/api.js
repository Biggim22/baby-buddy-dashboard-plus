...
async function localRequest(endpoint, options = {}) {
  const response = await fetchWithTimeout(`${LOCAL_API_BASE}/${endpoint}`, {
    headers: { "Content-Type": "application/json" },
    ...options,
  });
  if (!response.ok) {
...
export const api = {
  ...
  updateTask: (id, data) => localRequest(`tasks/${id}`, { method: "PATCH", body: JSON.stringify(data) }),
  ...
  updateLocalSettings: (childId, data) =>
    localRequest(`settings/${childId}`, { method: "PATCH", body: JSON.stringify(data) }),
};
