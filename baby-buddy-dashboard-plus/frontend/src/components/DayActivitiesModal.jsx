...
import { useTranslation, getLocale, getTimeFormat } from "../locales";
      ...
      return (
        <div style={{ display: "flex", flexDirection: "column" }}>
          {data.map((tummyEntry, i, arr) => (
            ...
            >
              <TimelineItem
                time={new Date(tummyEntry.start).toLocaleTimeString(getLocale(), { hour: "2-digit", minute: "2-digit", hour12: getTimeFormat() === "12h" })}
                label={`${Math.round(parseDuration(tummyEntry.duration) * 60)} ${t("common.min")}${tummyEntry.milestone ? ` · ${tummyEntry.milestone}` : ""}`}
                detail={`${new Date(tummyEntry.start).toLocaleTimeString(getLocale(), { hour: "2-digit", minute: "2-digit", hour12: getTimeFormat() === "12h" })} ${t("common.to")} ${new Date(tummyEntry.end).toLocaleTimeString(getLocale(), { hour: "2-digit", minute: "2-digit", hour12: getTimeFormat() === "12h" })}`}
                color={colors.tummy}
                isLast={i === arr.length - 1}
              />
            </div>
