import { translate as t, getLocale, getTimeFormat } from "../locales";
...
export function formatDueLabel(dueAt) {
  ...
  const time = dueAt.toLocaleTimeString(getLocale(), { hour: "2-digit", minute: "2-digit", hour12: getTimeFormat() === "12h" });
...
export function averageFeedingGapMs(feedings) {
  ...
  const sessions = feedings
    .map((f) => ({ start: new Date(f.start).getTime(), end: new Date(f.end || f.start).getTime() }))
    .filter((f) => Number.isFinite(f.start) && Number.isFinite(f.end))
    .sort((a, b) => a.start - b.start)
    .reduce((groups, feeding) => {
      const current = groups.at(-1);
      // A left/right breast switch is commonly saved as two entries. Treat entries
      // beginning within 15 minutes of the preceding end as one feeding session.
      if (current && feeding.start - current.end <= 15 * 60 * 1000) {
        current.end = Math.max(current.end, feeding.end);
      } else {
        groups.push({ ...feeding });
      }
      return groups;
  ...
  const times = sessions.map((session) => session.end);
  let totalGap = 0;
...
export function dailyFeedingTotals(entries, numDays = 30) {
  ...
  entries.forEach((e) => {
    ...
    if (key in sums) {
      ...
      durations[key] += parseDuration(e.duration) * 60;
      counts[key] += 1;
    }
  });
  const result = days.map((d) => ({
    date: d.label,
    amount: Math.round(sums[d.dateStr]),
    duration: Math.round(durations[d.dateStr]),
    count: counts[d.dateStr],
  }));
  const firstNonZero = result.findIndex((d) => d.amount > 0 || d.duration > 0 || d.count > 0);
