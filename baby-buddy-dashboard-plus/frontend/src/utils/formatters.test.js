...
describe("averageFeedingGapMs", () => {
  ...
  it("treats breast switches within 15 minutes as one feeding session", () => {
    const feedings = [
      { start: new Date(NOW.getTime() - 4 * 3600_000).toISOString(), end: new Date(NOW.getTime() - 3.8 * 3600_000).toISOString() },
      { start: new Date(NOW.getTime() - 3.75 * 3600_000).toISOString(), end: new Date(NOW.getTime() - 3.5 * 3600_000).toISOString() },
      { start: new Date(NOW.getTime() - 1.5 * 3600_000).toISOString(), end: new Date(NOW.getTime() - 1.25 * 3600_000).toISOString() },
    ];
    expect(averageFeedingGapMs(feedings)).toBe(2.25 * 3600_000);
  });
});
