...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if plus_reminder_task:
            plus_reminder_task.cancel()
            try:
                await plus_reminder_task
...
@app.get("/api/health")
async def health():
    """Supervisor watchdog endpoint; it also verifies that the local database is usable."""
...
async def proxy_baby_buddy(path: str, request: Request):
    ...
    try:
        ...
        logger.warning("Baby Buddy request failed: %s", exc)
...
async def proxy_media(path: str):
    ...
    try:
        ...
        logger.warning("Baby Buddy media request failed: %s", exc)
        raise HTTPException(502, "Cannot connect to Baby Buddy")
